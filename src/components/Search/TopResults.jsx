...
const TopResults = ({ data ,name }) => {
  ...
  return (
    <div>
      ...
      {
        data.map((artist,index)=><div key={index}> <ArtistList  key={index} artist={artist}/></div>)
      }
     </div>
    </div>
  );
  ...
  <div
    ...
    >
      ...
      <div className="absolute -bottom-1 md:h-[93px] bg-[#151515] w-full  ">
        <p className="m-auto md:text-4xl text-xl p-2 md:p-6 whitespace-nowrap">{artist?.name}</p>
      </div>
    </div>)
}